...
export const generateVideoSceneFromText = async (
    ...
    console.log("Veo operation finished:", operation.name, "Response:", operation.response);
...
export const generateRandomWorldSetting = async (): Promise<{
    ...
    const client = getAiClient();
    const randomTheme = RANDOM_WORLD_THEMES[Math.floor(Math.random() * RANDOM_WORLD_THEMES.length)];
    ...
    if (!worldDescription || worldDescription.trim() === "") {
        console.warn("Gemini API returned an empty world description for theme:", randomTheme);
        throw new Error("Gemini API returned an empty world description.");
    }

    const { videoUrl, videoMimeType } = await generateVideoSceneFromText(worldDescription.trim());
...
export const continueStoryInWorld = async (
  ...
  `;
    ...
    if (!nextSceneTextDescription || nextSceneTextDescription.trim() === "") {
        console.warn("Gemini API returned an empty continuation for action:", userAction);
        throw new Error("Gemini API returned an empty continuation.");
    }

    const { videoUrl, videoMimeType } = await generateVideoSceneFromText(nextSceneTextDescription.trim());
